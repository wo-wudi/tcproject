    ...
    <el-dialog
      ...
      <el-form :model="updateForm" status-icon :rules="rules" ref="updateForm">
        <el-form-item prop="uname">
          <el-input placeholder="请输入用户名" prefix-icon="el-icon-s-custom" v-model="updateForm.uname"></el-input>
        </el-form-item>
...
export default {
  data(){
    //判断输入的用户名格式
    var checkStuUName = (rule, value, callback) => {
      let phoneRegexp=/^[a-zA-Z0-9]{6,10}$/
      if (!value) {
    ...
    log(){
      ...
      this.axios.post("/user/u1/login",this.qs.stringify(object)).then(res => {
        ...
        if(this.res==0){
          ...
          //登录成功后将后台发送的用户名，用户id和token保存在vuex和localstorage中
